...
{!! Html::style(app('css').'/typesOfTypes.css') !!}
<!-- Start typesOfTypes -->
@if (count($typeOfTypes)>0)
<div class="navbar-fixed">
	<nav>
		<div class="nav-wrapper">
			<a href="javascript" class="brand-logo hide-on-large-only">{{trans('typeOfTypes.typeOfTypes')}}</a>
			<a href="javascript:;" data-activates="typesOfTypesForMobile" class="button-collapse show-typesOfTypes"><i class="material-icons">menu</i></a>
			<ul class="right hide-on-med-and-down">
				@if ($idOfTypeOfTypes==null)
					@foreach($typeOfTypes as $typesOfType)
						<li><a href='{{url()->current()}}/{{$typesOfType->id}}' class="waves-effect">{{$typesOfType->Type}}</a></li>
					@endforeach
				@elseif ($idOfTypeOfTypes!=null)
					@foreach($typeOfTypes as $typesOfType)
						<li><a href="{{$typesOfType->id}}" class="waves-effect">{{$typesOfType->Type}}</a></li>
					@endforeach
				@endif
			</ul>
			<ul class="side-nav" id='typesOfTypesForMobile'>
				@if ($idOfTypeOfTypes==null)
					@foreach($typeOfTypes as $typeOfTypes)
						<li><a href='{{url()->current()}}/{{$typeOfTypes->id}}' class="waves-effect">{{$typeOfTypes->Type}}</a></li>
					@endforeach
				@elseif ($idOfTypeOfTypes!=null)
					@foreach($typeOfTypes as $typeOfTypes)
						<li><a href="{{$typeOfTypes->id}}" class="waves-effect">{{$typeOfTypes->Type}}</a></li>
					@endforeach
				@endif
			</ul>
		</div>
